...
const NewsService = {
    ...
    // 新闻列表获取
    getList2:async ({pageNo,limit,value})=>{
        let skip=(pageNo-1)*limit;
        return value==0?NewsModel.find().skip(skip).limit(parseInt(limit)):
        NewsModel.find({category:value}).skip(skip).limit(parseInt(limit))
    },
    getTotal:async({value})=>{
        return value==0?NewsModel.countDocuments():NewsModel.find({category:value}).countDocuments();
    },
